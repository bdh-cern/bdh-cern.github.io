...
However, reality is not so simple. In fact, applying a current and generating a field actually changes the internal state of the magnet in an unpredictable way. This means that the next time the same current is applied, the field will be subtly different. 

The magnet field is therefore not only dependent on the presently-applied current, but also the previously-applied current (and the current before that, etc.). In other words, the magnet field has a *history dependence*. This history dependence is called 'magnetic hysteresis' or just 'hysteresis'.
